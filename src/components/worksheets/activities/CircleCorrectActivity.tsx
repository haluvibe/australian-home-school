          ...
          <div key={i} className="rounded-lg border border-charcoal/8 p-3">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {q.options.map((option, j) => (
                <div
                  key={j}
                  className="flex min-h-[48px] min-w-[48px] items-center gap-2 rounded-xl border-2 border-charcoal/15 bg-cream/50 px-4 py-2 text-lg font-semibold text-charcoal"
                >
                  <span className="inline-block h-5 w-5 shrink-0 rounded-full border-2 border-charcoal/30 print:border-charcoal/40" aria-hidden="true" />
                  {q.optionIcons?.[j] && (
