...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "Year 3 English Worksheets | AI Native School",
...
const strandIcons: Record<string, string> = {
  Language: "\u{1F524}",
  Literature: "\u{1F4DA}",
  Literacy: "\u{270F}\u{FE0F}",
};

export default function Year3EnglishWorksheetLanding() {
  const grouped = year3EnglishWorksheets.reduce<Record<string, typeof year3EnglishWorksheets>>(
    (acc, ws) => {
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-10 pt-14 sm:px-6 sm:pb-14 sm:pt-28 lg:px-8 lg:pb-16 lg:pt-32">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-charcoal-light sm:mt-6 sm:text-lg">
              Colourful, curriculum-aligned worksheets for Year 3 English.
              Covering prefixes, suffixes, grammar, punctuation, comprehension, recounts and more — just click, print and learn!
            </p>
          </div>
        </div>
      </section>
              ...
              <div key={strand}>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {worksheets.map((ws) => (
                    <Link
                      key={ws.slug}
                      href={`/worksheets/english/year-3/${ws.slug}`}
