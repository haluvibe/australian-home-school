...
import { year3EnglishWorksheets, getYear3EnglishWorksheet } from "@/lib/worksheet-data-english-year3";
...
export async function generateStaticParams() {
  return year3EnglishWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { itemSlug } = await params;
  const worksheet = getYear3EnglishWorksheet(itemSlug);
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Year 3 English Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/english/year-3/${itemSlug}` },
    openGraph: {
      title: `${worksheet.title} — Year 3 English Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/english/year-3/${itemSlug}`,
    },
  };
}
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((slug) => year3EnglishWorksheets.find((w) => w.slug === slug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year3-english-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/english/year-3" />
    </>
  );
}
