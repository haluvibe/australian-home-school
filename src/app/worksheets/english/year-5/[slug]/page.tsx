...
import { year5EnglishWorksheets, getYear5EnglishWorksheet } from "@/lib/worksheet-data-english-year5";
...
export async function generateStaticParams() {
  return year5EnglishWorksheets.map((ws) => ({ slug: ws.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const worksheet = getYear5EnglishWorksheet(slug);
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Year 5 English Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/english/year-5/${slug}` },
    openGraph: {
      title: `${worksheet.title} — Year 5 English Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/english/year-5/${slug}`,
    },
  };
}
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((nextSlug) => year5EnglishWorksheets.find((w) => w.slug === nextSlug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year5-english-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/english/year-5" />
    </>
  );
}
