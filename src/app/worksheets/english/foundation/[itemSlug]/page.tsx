...
import { foundationEnglishWorksheets, getFoundationEnglishWorksheet } from "@/lib/worksheet-data-english-foundation";
...
export async function generateStaticParams() {
  return foundationEnglishWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { itemSlug } = await params;
  const worksheet = getFoundationEnglishWorksheet(itemSlug);
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Foundation English Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/english/foundation/${itemSlug}` },
    openGraph: {
      title: `${worksheet.title} — Foundation English Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/english/foundation/${itemSlug}`,
    },
  };
}
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((slug) => foundationEnglishWorksheets.find((w) => w.slug === slug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`foundation-english-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/english/foundation" />
    </>
  );
}
