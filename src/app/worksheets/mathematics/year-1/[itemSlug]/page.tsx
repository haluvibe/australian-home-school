...
import { year1MathsWorksheets, getYear1Worksheet } from "@/lib/worksheet-data-year1";
...
export async function generateStaticParams() {
  return year1MathsWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { itemSlug } = await params;
  const worksheet = getYear1Worksheet(itemSlug);
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Year 1 Mathematics Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/mathematics/year-1/${itemSlug}` },
    openGraph: {
      title: `${worksheet.title} — Year 1 Mathematics Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/mathematics/year-1/${itemSlug}`,
    },
  };
}
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((slug) => year1MathsWorksheets.find((w) => w.slug === slug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year1-maths-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/mathematics/year-1" />
    </>
  );
}
