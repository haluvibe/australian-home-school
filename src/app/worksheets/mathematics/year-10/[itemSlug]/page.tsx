...
import { year10MathsWorksheets, getYear10Worksheet } from "@/lib/worksheet-data-year10";
...
export async function generateStaticParams() {
  return year10MathsWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { itemSlug } = await params;
  const worksheet = getYear10Worksheet(itemSlug);
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Year 10 Mathematics Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/mathematics/year-10/${itemSlug}` },
    openGraph: {
      title: `${worksheet.title} — Year 10 Mathematics Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/mathematics/year-10/${itemSlug}`,
    },
  };
}
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((slug) => year10MathsWorksheets.find((w) => w.slug === slug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year10-maths-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/mathematics/year-10" />
    </>
  );
}
