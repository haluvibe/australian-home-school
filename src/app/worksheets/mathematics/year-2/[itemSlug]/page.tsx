...
import { year2MathsWorksheets, getYear2Worksheet } from "@/lib/worksheet-data-year2";
...
export async function generateStaticParams() {
  return year2MathsWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!worksheet) return {};
  return {
    title: `${worksheet.title} — Year 2 Mathematics Worksheet`,
    description: worksheet.description,
    keywords: [`${worksheet.title}`, "free printable worksheet", "Australian Curriculum", "homeschool worksheet"],
    alternates: { canonical: `/worksheets/mathematics/year-2/${itemSlug}` },
    openGraph: {
      title: `${worksheet.title} — Year 2 Mathematics Worksheet | Australian Home Schooling`,
      description: worksheet.description,
      url: `/worksheets/mathematics/year-2/${itemSlug}`,
    },
  };
}
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year2-maths-${worksheet.slug}`} />
          </div>
        </div>
      </div>
