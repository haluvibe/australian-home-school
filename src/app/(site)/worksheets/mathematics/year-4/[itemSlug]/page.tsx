...
import { year4MathsWorksheets, getYear4Worksheet } from "@/lib/worksheet-data-year4";
...
export async function generateStaticParams() {
  return year4MathsWorksheets.map((ws) => ({ itemSlug: ws.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${worksheet.title} — Year 4 Mathematics Worksheet | AI Native School`,
      description: worksheet.description,
...
export default async function WorksheetItemPage({ params }: PageProps) {
  ...
  const nextStepWorksheets = (worksheet.nextSteps ?? [])
    .map((slug) => year4MathsWorksheets.find((w) => w.slug === slug))
  ...
  return (
    <>
      ...
      <div className="no-print border-b border-charcoal/5 bg-warm-white">
        <div className="mx-auto flex max-w-[210mm] items-center justify-between px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <PrintButton filename={`year4-maths-${worksheet.slug}`} />
          </div>
        </div>
      </div>
      ...
      <NextStepsSection nextSteps={nextStepWorksheets} basePath="/worksheets/mathematics/year-4" />
    </>
  );
}
