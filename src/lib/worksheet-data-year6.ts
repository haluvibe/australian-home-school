...
export const year6MathsWorksheets: WorksheetItem[] = [
  ...
  // ── WS 5: Decimal Metric Connections ──────────────────────
  {
    ...
    lesson: {
      ...
      parentTip: "Many children struggle to remember which direction to move the decimal when converting units — they know it is about powers of ten but get confused about multiply versus divide. A simple poster on the wall showing km → m → cm → mm with arrows saying ×1000, ×100, ×10 (and the reverse for dividing) gives a quick visual reference while the idea is being consolidated. Common misconception: children often move the decimal point the wrong direction when converting between metric units — they might multiply by 1000 when they should divide, or vice versa. A helpful rule: when converting from a larger unit to a smaller unit (metres to centimetres), the number gets bigger; when converting from smaller to larger (grams to kilograms), the number gets smaller. Another issue is that children may not connect decimal place value to metric prefixes — 0.5 m is 50 cm is 500 mm, because each step is a factor of 10. Your child is ready for the worksheet when they can: (1) explain the relationship between mm, cm, m, and km using powers of 10, (2) convert between adjacent metric units correctly, and (3) express a measurement as a decimal in a larger unit (e.g., 350 cm = 3.5 m).",
  ...
  // ── WS 8: Unknown Values & Combined Operations ───────────
  {
    ...
    lesson: {
      ...
      parentTip: "Children often find working backwards (using inverse operations) confusing at first because it feels counterintuitive. A helpful analogy is 'unwrapping a parcel': the last thing done to the unknown is the first thing you undo. If your child is stuck, encourage them to try a known number first — work through the equation forwards with a guess, see what comes out, then adjust. This builds intuition before the formal method. Common misconception: children often apply operations left to right without considering order of operations — they calculate 3 + 4 x 2 as 14 instead of 11. At Year 6, children need to understand that multiplication and division are performed before addition and subtraction. The mnemonic BODMAS (Brackets, Orders, Division/Multiplication, Addition/Subtraction) helps, but understanding WHY matters more than memorising the acronym. Another issue is that children struggle with multi-step problems that require working backwards — if 3 x ___ + 5 = 20, they need to subtract 5 first, then divide by 3. Your child is ready for the worksheet when they can: (1) correctly apply order of operations in a two-step equation, (2) solve for an unknown using inverse operations, and (3) check their answer by substituting it back into the original equation.",
  ...
  // ── WS 10: Algorithms to Generate Sets ────────────────────
  {
    ...
    lesson: {
      ...
      parentTip: "The concept of an algorithm may be unfamiliar by name, but your child already uses them every day — think of a recipe or morning routine. The key Year 6 idea is that algorithms must be precise and unambiguous. A great dinner-table activity is to have your child explain how to make a sandwich in exact steps; if any step is unclear, pretend you cannot proceed. This builds the habit of precision that underpins computational thinking. Common misconception: children often confuse following an algorithm (executing steps) with understanding why the algorithm works. They may correctly generate a set of numbers using a rule but not be able to explain the pattern or predict what will happen if the rule changes. Another issue is that children sometimes apply the algorithm inconsistently — they might add 3 for the first two steps but then add 4, losing track of the rule. Writing the rule at the top of their work and checking each step against it helps. Your child is ready for the worksheet when they can: (1) follow a given algorithm to generate a set of at least 6 numbers, (2) describe the algorithm in their own words, and (3) predict what would happen if one part of the algorithm changed.",
  ...
  // ── WS 12: Convert Length, Mass & Capacity ────────────────
  {
    ...
    lesson: {
      ...
      parentTip: "Measurement conversions are best learned by doing rather than memorising tables. Keep a small reference card on the fridge for the first few weeks, but aim to gradually wean your child off it by asking them to explain the reasoning rather than just look up the answer. If your child consistently divides when they should multiply (or vice versa), ask: 'Are we going to a bigger unit or a smaller unit?' Smaller units means more of them, so we multiply. Common misconception: children frequently confuse which operation to use when converting — 'Do I multiply or divide?' The simple rule is: converting to a SMALLER unit makes a BIGGER number (multiply), and converting to a BIGGER unit makes a SMALLER number (divide). So 2.5 kg becomes 2,500 g (multiply by 1,000), and 450 mL becomes 0.45 L (divide by 1,000). Children also commonly forget the conversion factors themselves — keeping a reference card visible during practice builds fluency. Your child is ready for the worksheet when they can: (1) recall the conversion factors for common metric units (1 km = 1,000 m, 1 kg = 1,000 g, 1 L = 1,000 mL, 1 m = 100 cm), (2) convert between units in both directions, and (3) solve a word problem that requires a unit conversion as part of the solution.",
  ...
  // ── WS 14: Angle Properties ───────────────────────────────
  {
    ...
    lesson: {
      ...
      parentTip: "Angles on a straight line summing to 180° and angles around a point summing to 360° are rules children need to be able to recall quickly. A kinesthetic trick: have your child stand and turn a full circle — that is 360°. Half a turn is 180°. Tie this to real objects: a straight wall edge represents 180°, a clock face represents 360°. For triangles, tearing the three corners off a paper triangle and arranging them in a row to show a straight line (180°) is a memorable demonstration. Common misconception: children often memorise that 'angles on a straight line add to 180' but do not recognise straight lines in context — if the line is drawn vertically or at a diagonal, they may not apply the rule. Another common error is confusing supplementary angles (add to 180) with complementary angles (add to 90). Children also frequently make arithmetic errors when finding an unknown angle because they subtract from the wrong total. Teach them to always state the rule first, then substitute, then calculate. Your child is ready for the worksheet when they can: (1) state and apply the rule that angles on a straight line sum to 180 degrees, (2) state and apply the rule that angles at a point sum to 360 degrees, (3) calculate an unknown angle by subtracting known angles from the appropriate total, and (4) identify vertically opposite angles as equal.",
  ...
  // ── WS 15: Parallel Cross-Sections of Right Prisms ───────
  {
    ...
    lesson: {
      ...
      parentTip: "Cross-sections are much easier to understand with actual objects in hand. Kitchen vegetables are perfect: a carrot cut at different angles gives different cross-sections, a cucumber is a right prism (circular cross-sections), and a block of cheese is a rectangular prism. Let your child predict the cross-section shape before you make the cut — prediction followed by checking is very effective for building geometric intuition. Common misconception: children often assume that all cross-sections of a 3D object are the same shape, regardless of where the cut is made. In reality, a horizontal cross-section of a cone gives a circle, but a vertical cross-section gives a triangle. Another issue is that children struggle to visualise a cross-section without physically cutting an object — which is why this lesson works so much better with real objects (fruit, Play-Doh, cheese) than with diagrams alone. Your child is ready for the worksheet when they can: (1) predict the shape of a cross-section for a cube, cylinder, and cone, (2) explain how the cross-section changes depending on where and how the cut is made, and (3) identify parallel cross-sections as cross-sections that are the same shape along the length of a prism.",
  ...
  // ── WS 16: Tessellations & Transformations ────────────────
  {
    ...
    lesson: {
      ...
      parentTip: "Tessellations are a wonderful bridge between mathematics and art, and many children become highly engaged when they can see the creative potential. Encourage your child to notice tessellating patterns around the home — tiles, brick walls, fabric patterns. If they struggle to identify which transformation they used, suggest they trace the original shape and physically slide, flip, or turn it over the pattern to find the match. Common misconception: children often think any shape can tessellate — they may not realise that regular pentagons, for example, cannot tile a plane without gaps. The key insight is that the angles meeting at each vertex must sum to exactly 360 degrees. Another common confusion is between translation (sliding), rotation (turning), and reflection (flipping) — children may use these terms interchangeably. Practising each transformation physically with a cut-out shape on paper makes the distinctions concrete. Your child is ready for the worksheet when they can: (1) demonstrate or describe what translation, rotation, and reflection mean, (2) create a tessellation using at least one shape and explain why it works, and (3) identify which transformation was used to move a shape from one position to another.",
