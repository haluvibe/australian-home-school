...
export const year2EnglishWorksheets: WorksheetItem[] = [
  ...
  // ── WS 4: Joining Words (Conjunctions) ─────────────────────────
  {
    ...
    lesson: {
      ...
      parentTip:
        "Children at this stage often overuse 'and' to join every idea in a run-on sentence. Gently point this out when it happens: 'That is a great idea — which joining word shows the contrast between those two things?' Offer 'but' as an option. Over time, they will begin to choose deliberately. Common misconception: children frequently confuse 'but' and 'because' — they might write 'I was happy because I lost my toy' when they mean 'but'. Help them by asking: 'Does the second part give a reason (because) or show something different from what we expected (but)?' Another common error is starting sentences with 'Because' and not finishing them — 'Because it was raining.' is a fragment, not a sentence. Show them that 'because' needs two parts: the reason and the result. Your child is ready for the worksheet when they can: (1) correctly choose between 'and', 'but', and 'because' to complete a sentence at least 3 out of 4 times, (2) explain in simple terms what each joining word does (and = adds, but = contrasts, because = gives a reason), and (3) write at least one original compound sentence using a joining word.",
      extension:
