...
export const foundationMathsWorksheets: WorksheetItem[] = [
  ...
  // 4. Partition and combine
  {
    ...
    lesson: {
      ...
      parentTip: "Partitioning is the foundation of all future addition — if children can see that 5 is '3 and 2' or '4 and 1', mental arithmetic becomes intuitive. Use physical objects every time; abstract numbers come later. There is no rush. Common misconception: children often think there is only one way to partition a number — they learn that 5 is '2 and 3' and do not realise it is also '1 and 4' or '0 and 5'. Encourage flexibility by asking: 'Can you show me another way to make 5?' Another issue is that children may not yet understand that the parts must add up to the whole — they might say 5 is '3 and 3' because both numbers feel close. Use physical objects so they can see and count. Your child is ready for the worksheet when they can: (1) split a number into two parts using objects and say both parts aloud, (2) show at least two different ways to partition the same number, and (3) put two groups of objects together and say how many there are in total.",
  ...
  // 5. Add, subtract, share
  {
    ...
    lesson: {
      ...
      parentTip: "At Foundation level, the words 'add', 'take away', and 'share' each need to be experienced physically before they become symbols. If your child is confused by the written question, act it out with objects first — every time. Concrete before abstract. Common misconception: children often think 'take away' always means the answer is smaller — they may resist the idea that adding zero keeps the number the same. For sharing, children frequently deal out objects unevenly because they lose track of turns. The 'one for you, one for you, one for you' dealing method is the most reliable at this age. Another common issue is that children interpret 'share between 2' as 'give 2 away' rather than 'split into 2 equal groups'. Your child is ready for the worksheet when they can: (1) add two small groups of objects together and say the total, (2) take away objects from a group and count what remains, and (3) share a group of objects equally between 2 people using the dealing method.",
  ...
  // 6. Repeating patterns
  {
    ...
    lesson: {
      ...
      parentTip: "Patterns feel obvious to adults but are a genuine discovery for young children. If your child breaks the pattern, don't immediately correct them — ask 'What comes next? Let's check by clapping it.' Rhythm helps enormously: clap-clap-stomp is easier to continue than looking at coloured blocks. Common misconception: children often continue a pattern correctly for a few steps but then lose track of the repeating unit — for example, in a red-blue-red-blue pattern, they might suddenly add a green because they forget which colour comes next. The key skill is identifying the 'core' of the pattern (the part that repeats) and holding it in mind. Another issue is that children sometimes copy the pattern rather than extend it — they recreate what is already shown instead of adding the next elements. Your child is ready for the worksheet when they can: (1) identify the repeating unit in a simple AB or ABB pattern, (2) correctly extend a pattern by at least 3 elements, and (3) create their own simple repeating pattern using objects or colours.",
  ...
  // 8. Direct comparison
  {
    ...
    lesson: {
      ...
      parentTip: "Direct comparison is about understanding what 'fair' measurement means — you need to line objects up at the same starting point. If your child always starts from different ends, show them a running race start line as a real-world analogy: 'Everyone starts from the same spot, or it's not fair!' Common misconception: children often compare objects without aligning them properly — they might say a pencil is longer than a crayon even when the pencil starts further along the table. The concept of a 'fair start' (lining up one end) is crucial and not intuitive for all children. Another common issue is that children confuse 'taller' with 'bigger' — a tall thin bottle might hold less than a short wide cup, but children will say the bottle is 'bigger'. At this stage, focus on one attribute at a time (length, mass, capacity) and make comparisons explicit. Your child is ready for the worksheet when they can: (1) line up two objects at the same starting point and identify which is longer, (2) hold two objects and say which is heavier, and (3) use comparison language correctly: longer/shorter, heavier/lighter, holds more/holds less.",
  ...
  // 9. Time of day
  {
    ...
    lesson: {
      ...
      parentTip: "Children understand time through routine long before they can read a clock face. Anchor today's lesson to your own daily schedule — 'After breakfast we do maths, after lunch we rest' gives abstract time words a real meaning they can feel. Common misconception: children often confuse the sequence of time periods — they may place 'evening' before 'afternoon' or not understand that 'night' is when they sleep. The concept of time as a cycle that repeats every day is not obvious to all Foundation-age children. Another common issue is that children anchor events to the wrong time of day because their personal routine differs from the examples (e.g., a child who eats dinner at 5pm may associate dinner with 'afternoon' rather than 'evening'). Accept their personal experience as valid and discuss how different families have different routines. Your child is ready for the worksheet when they can: (1) name the four parts of the day in order (morning, afternoon, evening, night), (2) sort at least 4 daily events into the correct time of day, and (3) describe their own daily routine using time-of-day language.",
  ...
  // 11. Position and location
  {
    ...
    lesson: {
      ...
      parentTip: "Positional language develops through movement and play — not through worksheets alone. Before this lesson, play 'Simon Says' with position words: 'Simon says put the ball UNDER the chair, BEHIND the door, NEXT TO the cup.' This warm-up makes the lesson concepts click. Common misconception: children often confuse 'left' and 'right' — this is developmentally normal and can persist well into Year 1 or 2. Use a consistent physical anchor (e.g., 'your writing hand is your right hand'). Another common issue is that children describe position from their own perspective and do not realise that 'left' and 'right' depend on which way you are facing. For Foundation, accept the child's perspective and do not introduce the idea of relative directions yet. Also, 'between' is surprisingly difficult — children often point to one of the objects rather than the space in the middle. Your child is ready for the worksheet when they can: (1) correctly follow at least 3 positional instructions (put the block on top of, beside, under, behind, in front of), (2) describe where an object is using positional language, and (3) understand 'between' as the space in the middle of two objects.",
  ...
  // 12. Collect, sort, compare data
  {
    ...
    lesson: {
      ...
      parentTip: "Data collection is most meaningful when the question comes from the child. If they're not engaged, ask them to choose what to investigate: 'Do you want to find out what colour cars go past, or what everyone's favourite fruit is?' Their curiosity drives the learning. Common misconception: children often sort objects into groups based on personal preference rather than a shared attribute — they might put all their favourite toys in one group regardless of colour, size, or type. The concept of sorting by a consistent rule ('all the red ones here, all the blue ones there') needs explicit modelling. Another issue is that children may count the groups rather than the objects in each group — asking 'How many red blocks?' and getting '1' (one group) instead of '4' (four blocks). Your child is ready for the worksheet when they can: (1) sort a collection of objects into groups based on one attribute (colour, shape, or size), (2) explain the rule they used for sorting, and (3) count and compare the number of objects in each group.",
